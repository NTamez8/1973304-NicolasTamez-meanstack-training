...
class ShoppingCart {
    ...
    buildCartFromJSON(NewCart) {
        this.Cart = new Map(JSON.parse(NewCart));
    }
    addItemToList(item) {
        let id = item.getId();
        if (!this.Cart.has(id)) {
...
function BuildStock() {
    let myApi = new ShoppingAPI();
    let Stock = myApi.GetItems();
...
function BuildFooter(totalPrice) {
    let CurrencyFormatter = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' });
    let body = document.getElementById("TableBody");
    let row = document.createElement("tr");
